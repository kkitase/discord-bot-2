...
- **スケジュールリマインダー:** 毎日のスケジュールを自動でお知らせします。（環境変数で ON/OFF 可能）
...
3.  **環境変数を設定します:**
    ...
    ```
    - **REMINDER_ENABLED**: スケジュールリマインダー機能の有効/無効を切り替えます (`true` または `false`)。設定しない場合は無効になります。
...
3.  **Cloud Run へのデプロイ:**
    プロジェクトのルートで以下のコマンドを実行します。リマインダー機能を有効にする場合は `--set-env-vars=REMINDER_ENABLED=true` を追加してください。
    ```bash
    gcloud run deploy discord-bot \
      ...
      --set-env-vars=REMINDER_ENABLED=true
    ```
