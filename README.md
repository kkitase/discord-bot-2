...
- **スケジュールリマインダー:** 毎日のスケジュールを自動でお知らせします。
- **スラッシュコマンド:** ボットの動作をコマンドで制御できます。

## 🤖 スラッシュコマンド

- `/mute status:<On|Off>`: ボットの自動応答（相槌やリアクション）をオン/オフします。メンションへの返信とウェルカムメッセージは、ミュート中も機能します。
...
2.  **シークレットの保管:**
    ...
    **方法1: .env ファイルから一括設定（推奨）**
    ...
    gcloud secrets create discord-reminder-channel-id --replication-policy="automatic"
    gcloud secrets create discord-welcome-channel-id --replication-policy="automatic"
    gcloud secrets create discord-intro-channel-id --replication-policy="automatic"
    
    # .env から読み込んだ値をシークレットに設定
    printf "$DISCORD_BOT_TOKEN" | gcloud secrets versions add discord-bot-token --data-file=-
    printf "$DISCORD_CLIENT_ID" | gcloud secrets versions add discord-client-id --data-file=-
    printf "$GEMINI_API_KEY" | gcloud secrets versions add gemini-api-key --data-file=-
    printf "$DISCORD_REMINDER_CHANNEL_ID" | gcloud secrets versions add discord-reminder-channel-id --data-file=-
    printf "$DISCORD_WELCOME_CHANNEL_ID" | gcloud secrets versions add discord-welcome-channel-id --data-file=-
    printf "$DISCORD_INTRO_CHANNEL_ID" | gcloud secrets versions add discord-intro-channel-id --data-file=-
    ```
    ...
    **方法2: 手動で1つずつ設定**
    ...
    printf "YOUR_DISCORD_CLIENT_ID_HERE" | gcloud secrets versions add discord-client-id --data-file=-
    ...
    printf "YOUR_REMINDER_CHANNEL_ID" | gcloud secrets versions add discord-reminder-channel-id --data-file=-
    printf "YOUR_WELCOME_CHANNEL_ID" | gcloud secrets versions add discord-welcome-channel-id --data-file=-
    printf "YOUR_INTRO_CHANNEL_ID" | gcloud secrets versions add discord-intro-channel-id --data-file=-
    ```
...
3.  **Cloud Run へのデプロイ:**
    ...
    gcloud run deploy discord-bot-2 \
      ...
      --update-secrets=DISCORD_CLIENT_ID=discord-client-id:latest \
      --update-secrets=GEMINI_API_KEY=gemini-api-key:latest \
      --update-secrets=DISCORD_REMINDER_CHANNEL_ID=discord-reminder-channel-id:latest \
      --update-secrets=DISCORD_WELCOME_CHANNEL_ID=discord-welcome-channel-id:latest \
      --update-secrets=DISCORD_INTRO_CHANNEL_ID=discord-intro-channel-id:latest
    ```
